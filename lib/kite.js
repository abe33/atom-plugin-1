...
let child_process, CompositeDisposable, TextEditor, AccountManager,
  Logger, completions, signatures, metrics, KiteApp,
...
const Kite = module.exports = {
  ...
  activate() {
    ...
    if (!signatures) { signatures = require('./signatures'); }
    signatures.init(this);
    this.subscriptions.add(signatures);
    ...
    this.subscriptions.add(atom.commands.add('atom-text-editor:not([mini])', {
      'core:cancel'() { signatures.hideOverlay(); },
    }));
