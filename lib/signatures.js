...
module.exports = class KiteSignature {
    ...
    const intersectionObserver = new IntersectionObserver(([entry]) => {
      ...
      if (this.listElement.parentNode && !mutationObserverDisposable) {
        ...
        if (this.marker) {
          ...
          this.signature.classList.remove('fixed');
      ...
      if (data) {
        if (!this.signature) {
          ...
          this.signature.onDidAttach = () => {
            ...
            this.scrollDisposable.add(view.onDidChangeScrollTop(() => {
              this.requestOverflowCheck();
            }));
  ...
  requestOverflowCheck() {
    ...
    requestAnimationFrame(() => {
      ...
      const scrollBottom = scrollTop + view.getHeight();
      const sigBounds = this.signature.getBoundingClientRect();
      const upsideDown = this.signature.classList.contains('upside-down');
      const shouldFollowCompletion = this.completionVisible && atom.config.get('kite.signatureFollowCompletion');

      // We have a marker so the signature is visible
      if (this.marker) {
        // Both the completion and signature panels are visible and
        // the setting is enabled
        if (shouldFollowCompletion) {
          // measuring the available space depending on whether the signature
          // is placed below or not.
          const availableSpace = upsideDown
            ? scrollBottom - pixelPos.top
            : pixelPos.top - scrollTop;

          // There's not enough space
          if (sigBounds.height > availableSpace) {
            // placing the signature in a fixed position
            this.signature.classList.add('fixed');

            this.stickToCompletions();
            let frameRequested;
            ...
            this.resizeObserverDisposable = new CompositeDisposable();

            this.resizeObserverDisposable.add(new DisposableEvent(window, 'resize', () => {
              this.stickToCompletions();
            }));
            this.resizeObserverDisposable.add(new Disposable(() => {
              resizeObserver.disconnect();
              this.signature.classList.remove('fixed');
              this.signature.style.width = '';
              this.signature.style.top = '';
              this.signature.style.left = '';
            }));

          } else {
            this.resizeObserverDisposable && this.resizeObserverDisposable.dispose();
            delete this.resizeObserverDisposable;
          }

        // Normal behaviour, when no completions are visible, is to move
        // the signature below the cursor if there isn't enough space above
        } else {
          this.resizeObserverDisposable && this.resizeObserverDisposable.dispose();
          delete this.resizeObserverDisposable;
  ...
  stickToCompletions() {
    ...
    // We're getting there when switching tabs, and then the editor view
    // no longer exists.
    // We also get there after switching back from fixed, and we don't wan't
    // to rewrite the position.
    if (!view || !this.signature.classList.contains('fixed')) { return; }

    const minWidth = 400;
    const pixelPos = view.pixelPositionForBufferPosition(this.activeEditor.getCursorBufferPosition());
    const viewBounds = view.getBoundingClientRect();
    const compBounds = this.listElement.getBoundingClientRect();
    const sigBounds = this.signature.getBoundingClientRect();
    const compOnTop = viewBounds.top < pixelPos;

    const spaceRight = window.innerWidth - compBounds.right;
    const spaceLeft = compBounds.left;
    const spaceBottom = window.innerHeight - compBounds.bottom;
    const spaceTop = compBounds.top;

    // We want to position the panel where we have the most space available
    if (sigBounds.width <= spaceRight) {
      // First, on the right
      this.signature.style.top = `${Math.min(compBounds.top, viewBounds.bottom - sigBounds.height)}px`;
      this.signature.style.left = `${compBounds.right}px`;
    } else if (spaceRight >= minWidth) {
      // then, on the right but resized
      this.signature.style.top = `${Math.min(compBounds.top, viewBounds.bottom - sigBounds.height)}px`;
      this.signature.style.left = `${compBounds.right}px`;
      this.signature.style.width = `${spaceRight}px`;

    } else if (sigBounds.width <= spaceLeft) {
      // Then on the left
      this.signature.style.top = `${Math.min(compBounds.top, viewBounds.bottom - sigBounds.height)}px`;
      this.signature.style.left = `${compBounds.left - sigBounds.width}px`;
    } else if (spaceLeft >= minWidth) {
      // then, on the left but resized
      this.signature.style.top = `${Math.min(compBounds.top, viewBounds.bottom - sigBounds.height)}px`;
      this.signature.style.left = `${compBounds.left - sigBounds.width}px`;
      this.signature.style.width = `${spaceLeft}px`;

    } else if (sigBounds.height <= spaceBottom && !compOnTop) {
      // Then below if suggestions are also below
      this.signature.style.top = `${compBounds.bottom}px`;
      this.signature.style.left = `${compBounds.left}px`;

    } else if (sigBounds.height <= spaceTop && compOnTop) {
      // Then above if suggestions are also above
      this.signature.style.top = `${compBounds.top - sigBounds.height}px`;
      this.signature.style.left = `${compBounds.left}px`;

    } else {
      // Not enough space in the three direction we could use.
      this.signature.style.top = `${Math.min(compBounds.top, viewBounds.bottom - sigBounds.height)}px`;
      this.signature.style.left = `${compBounds.right}px`;
      this.signature.style.width = `${spaceRight}px`;
    }
  ...
  checkCompletionOverlap() {
    if (this.decoration && this.completionVisible && atom.config.get('kite.signatureFollowCompletion')) {
      const sigBounds = this.signature.getBoundingClientRect();
      const fixed = this.signature.classList.contains('fixed');
      const listBounds = this.listElement.getBoundingClientRect();

      if (!fixed && !(sigBounds.top > listBounds.bottom || sigBounds.bottom < listBounds.top)) {
